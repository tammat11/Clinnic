        ...
        >
            <div className="w-full flex items-center justify-between h-16 md:h-20 px-4 sm:px-10 lg:px-12 bg-inherit rounded-[inherit] border border-transparent shadow-none transition-all duration-300 relative gap-4">
                ...
                <div className="flex-1 overflow-x-auto scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 flex justify-start md:justify-center">
                    <div className="flex items-center gap-2 md:gap-10 bg-slate-100/80 md:bg-slate-100/50 px-4 py-2 md:px-8 md:py-3 rounded-full border border-white/40 shadow-sm md:shadow-inner whitespace-nowrap backdrop-blur-md">
                        {links.map((link) => (
                            <a
                                key={link.name}
                                href={link.href}
                                className="text-xs md:text-sm font-bold text-slate-600 hover:text-[#007f94] transition-colors relative group px-2 py-1"
                            >
                                {link.name}
                            </a>
                        ))}
